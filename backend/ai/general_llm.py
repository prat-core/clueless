...
Available tools:
1. NAVIGATION_TOOL - Use when user wants to navigate somewhere on the website or navigation is required in order to complete their task (e.g., "how to change my email", "find product page", "how to buy", "navigate to login")
...
Respond with ONLY the tool name: either "NAVIGATION_TOOL" or "RAG_TOOL"."""
    ...
    def classify_query(self, user_query: str) -> str:
        ...
        if self.navigation_agent:
            try:
                logger.info(
                    f"🧠 Classifying query using Smart Navigation Agent: {user_query[:100]}..."
                )
            ...
            logger.warning(
                "⚠️ Navigation agent not available, using keyword-based classification"
            )
            return self._classify_by_keywords(user_query)
    ...
    def _classify_by_keywords(self, user_query: str) -> str:
        ...
        for keyword in navigation_keywords:
            if keyword in query_lower:
                logger.info(
                    f"🔀 Query classified as NAVIGATION_TOOL (keyword: {keyword})"
                )
    ...
    def route_query(
        self, user_query: str, current_url: Optional[str] = None
    ) -> Dict[str, Any]:
        """
